...
knitr::opts_chunk$set(echo = TRUE,message = FALSE,warnung=FALSE,comment = NA)
```
...
```{r}
set.seed(42)
#generate 30 observation for x
X <- seq(-10,10,length.out=30)
U <- - X
cor(X,X+rnorm(30))
plot(X,X)

#create difference linear relationship
...
df <- data.frame(Linear,Sine,Exponential,Quadratic,Logarithmic)
head(df)
...
```{r}
cbind(df,predicted=predict(model,test),resid(model))
```
